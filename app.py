...
def diag():
    ...
    # 1. Descobrir IP real de saída do Railway
    try:
        r = req.get('https://api.ipify.org?format=json', timeout=5)
        results['outbound_ip'] = r.json().get('ip', 'unknown')
    except Exception as e:
        results['outbound_ip'] = f'error: {e}'
 
    # 2. Testar chamada ao Absolute com assinatura
    ...
    try:
        ...
        results['absolute_status'] = r.status_code
        results['absolute_body'] = r.text[:300]
    except Exception as e:
        results['absolute_error'] = str(e)
